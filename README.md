...
- **Benchmarks** — race up to 5 models (local + cloud, mixed) through **standardized or custom benchmark suites** and compare them with live charts: a leaderboard, accuracy, a per-request encode/queue/prefill/decode breakdown, latency distributions, decode + prefill throughput, TPOT and inter-token jitter, radar profiles, an accuracy-vs-speed scatter, a per-task heatmap, self-reported cost estimates, a sortable/filterable task matrix, and a cross-run History view. Six built-in auto-scored suites, deliberately hard so models actually separate (incl. long-context retrieval and timing-only Speed & Latency); custom suites support deterministic scoring, an LLM-as-judge mode, timing-only tasks, and multi-turn prompts
...
- **Standardized suites** — six built-in, deterministically auto-scored suites, tuned to *separate* models rather than to be passed:
  ...
  - **Reasoning & Math** (12) — multi-step word problems, constraint puzzles, and traps where one slip changes the answer. The sharpest separator in the set.
  - **General Knowledge** (12) — multiple choice with plausible distractors and planted misconceptions (the universal *plasma* donor is not the universal red-cell donor).
  - **Instruction Following** (10) — exact word counts, a forbidden letter, precise separators, nested JSON with computed values, an acrostic, and a multi-turn recall. Tests control of the output; a model that adds a preamble scores zero.
  - **Long Context & Retrieval** (8) — questions over ~2,500-token logs: find a buried fact, resist near-identical distractors, aggregate figures scattered across the document, and answer `NOT FOUND` when the fact genuinely isn't there.
