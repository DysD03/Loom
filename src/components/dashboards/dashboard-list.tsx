...
import { LayoutDashboard, MoreHorizontal, Pencil, Plus, Trash2 } from "lucide-react";
...
import { deleteDashboardAction, renameDashboardAction } from "@/app/dashboards/actions";
...
export function DashboardList({
  ...
  function commitRename(id: string) {
    ...
    if (next) {
      startTransition(async () => {
        await renameDashboardAction(id, next);
        router.refresh();
      });
    }
  }
            ...
            {dashboards.map((dashboard) => {
              ...
              return (
                <li key={dashboard.id} className="group relative">
                  <button
                    type="button"
                    onClick={() => router.push(`/dashboards?d=${dashboard.id}`)}
                    className={cn(
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem
                        onClick={() => {
                          editValue.current = dashboard.title;
